...
// Raw research material: fetched PDFs and saved HTML pages, about 300 MB of
// it. It is how the figures and the fact-checks were sourced, but it is not
// part of the lecture and has no business in a public repository. The curated
// briefings and the data the charts read are kept.
const SKIP_PATHS = [
  'slides/plateaus-to-paradigms/research/scratch',
  // competition-101 is a class-meeting talk, not part of the published lecture
  // series, and it still has slides marked 待补. Remove this line to publish it.
  'slides/competition-101',
  // Not part of this repo at all: a video-generation service that lives in
  // the same directory and carries its own credentials.
  'h3',
]
...
function copyTree(from, to, rel = '') {
  ...
  for (const entry of fs.readdirSync(from, { withFileTypes: true })) {
    ...
    if (SKIP_PATHS.some(p => next === p || next.startsWith(p + '/'))) continue
    const src = path.join(from, entry.name)
    const dst = path.join(to, entry.name)
    if (entry.isDirectory()) copyTree(src, dst, next)
